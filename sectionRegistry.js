...
function renderPage(pageData){
  ...
  // Render HeadStat if present
  if(pageData.headStat){

    const layout = pageData.headStat.layout;
    const renderer = HeadStatLayouts[layout];
  ...
  // Render sections
  html += renderSections(pageData);

  html += `</div>`;

  container.innerHTML = html;
  populatePage(pageData);
  renderChart(pageData);   // ← THIS IS REQUIRED
